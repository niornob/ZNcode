...
\(\Lambda\) of a closed oriented \(d\)-manifold. We allow general local
\(X\)- and \(Z\)-type source terms and study the thermal ensemble itself,
rather than a decoding model or disorder average. Our central objects are
...
\end{equation}
The groups $H_m$ and $H_e$ label the magnetic and electric background charges respectively, i.e., they are the groups in which the background charges \([q_m]\) and \([q_e]\) take values.
...
When the Fourier-resolved representation \eqref{eq:Zlmle} is available, the corresponding
sector amplitudes \(\widehat{\mathcal Z}_M(\ell_m,\ell_e;q_m,q_e)\) have generally complex
one-polymer activities \eqref{eq:fixed-background-unified-polymer-activity}, and
opposite-species polymers interact through the pure phase \eqref{eq:MEphase}. We use these
sector amplitudes as a convenient intermediate device, but the main conclusion
of the section is a background-uniform low-activity region for the physical fixed-background
partition functions \(\mathcal Z_M(q_m,q_e)\). In that region, these partition functions are
absolutely controlled uniformly in finite volume, large connected polymers are
exponentially suppressed, and homologically nontrivial polymers are exponentially
suppressed on the scale of the corresponding spacetime systole. Here \emph{systole} means
the minimal support size of a nontrivial homology cycle. The weakest input in the
derivation of this region is the available combinatorial bound on the number of rooted
connected closed polymers: the present boundary is controlled not by the abstract polymer
criterion itself, but by the quality of this counting estimate.
...
\begin{equation}
	...
	=
	\prod_{u\in C_P(\underline\Lambda)}
	\chi_{(q_e+D^{\mathsf T}\lambda)_u}(\phi_u),
\end{equation}
...
\begin{equation}
	\begin{aligned}
		...
		\Biggl[
		\sum_{x\in \mathbb Z_N}
		V_u(x)\,\chi_{(q_e+D^{\mathsf T}\lambda)_u}(x)
		\Biggr].
	\end{aligned}
	\label{eq:KW-step2}
\end{equation}
...
\begin{aligned}
	...
	\Biggl(
	\prod_{u\in C_P(\underline\Lambda)}
	\widehat V_u \left(-(D^{\mathsf T}\lambda+q_e)_u\right)
	\Biggr),
\end{aligned}
...
\begin{equation}
	(D^{\mathsf T}\lambda+q_e)_{\theta_{\underline\Lambda}(u^\vee)}
	=
